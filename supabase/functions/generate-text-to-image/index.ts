...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Starting FLUX text-to-image generation...');
    console.log('Prompt:', prompt);

    // Submit generation request to BFL API using text-to-image endpoint
    const submitResponse = await fetch('https://api.bfl.ml/v1/flux-schnell', {
      method: 'POST',
      headers: {
